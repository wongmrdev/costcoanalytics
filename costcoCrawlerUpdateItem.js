...
function regexDateValidUpdateOnScan(regex, dateTransformer) { //Constructor to take in any regex and it's helper date transformation function and output a callback to DynamoDB.scan(err, callback)
    return function (err, data) { //return a dynamic function to call in scan
        if (err) {
            ...
            data.Items.filter(coupon => regex.test(coupon.dateValid))
                .forEach(function (coupon) {
                    console.log("updating: " + coupon.id + ";", coupon.dateValid, ";", coupon.createdAt);
                    //match dateValid to regex of interest (parameter), then transform
                    let regexExecGroups = regex.exec(coupon.dateValid)
                    const { beginMonth, endMonth, beginDay, endDay, beginYear, endYear } = dateTransformer(regexExecGroups)
                    console.log("newDateValid " + beginMonth + "/" + beginDay + "/" + beginYear + " - "
                        + endMonth + "/" + endDay + "/" + endYear)
                    let newDateValid = "Valid " + beginMonth + "/" + beginDay + "/" + beginYear + " - "
                        + endMonth + "/" + endDay + "/" + endYear  //Transformed DateValid
                    ...
                    docClient.update(updateParams, function (err, data) {
                        if (err) console.log("error in the updateItem", err, err.stack); // an error occurred
                        else console.log(`successfully updated to: ${JSON.stringify(data)}`);
                    })
                });
            ...
            if (typeof data.LastEvaluatedKey != "undefined") {
                ...
                docClient.scan(params, regexDateValidUpdateOnScan(regex, dateTransformer));
            }
        }
    }
}
...
function getYearNumberFromMonthAsLetters(monthAsLetters) {
    ...
    switch (monthAsLettersLower) {
        ...
        case "nov": return "11";
        case "dec": return "12";
        default: return "no matching month";
    }
}
